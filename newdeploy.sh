...
# 運行新的容器
echo "運行新的容器..."
docker run -idt --name img2tag --network="host" -e RABBITMQ_HOST=***.***.*.*** -e RABBITMQ_PORT=5672 -e RABBITMQ_USER=james -e RABBITMQ_PASSWORD=*************** -v /var/www/html/idj_AMS/uploads:/uploads -v /var/www/Img2Tag/log:/app/data/log img2tag:latest
